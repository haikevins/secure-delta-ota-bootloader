                        ...
                        UpdateHandoffRecord_t *record)
{
    return (uint8_t)ExternalFlashStorage_Read(
        PartitionForSlot(slot), 0UL, (uint8_t *)record, sizeof(*record));
}
    ...
    UpdateHandoffSlot_t *slot)
{
    ...
    {
        if (slot != (UpdateHandoffSlot_t *)0) { *slot = newest; }
    ...
    UpdateHandoffSlot_t *written_slot)
{
    ...
    if (current == UPDATE_HANDOFF_SLOT_A) { current_generation = a.generation; }
    else if (current == UPDATE_HANDOFF_SLOT_B) { current_generation = b.generation; }
    ...
    if (!ExternalFlashStorage_Write(partition, 0UL,
                                    (const uint8_t *)&candidate,
                                    crc_offset) ||
        !ExternalFlashStorage_Write(partition, crc_offset,
    ...
    if (!ExternalFlashStorage_Read(partition, 0UL,
    ...
    if (committed != (UpdateHandoffRecord_t *)0) { *committed = verify; }
    if (written_slot != (UpdateHandoffSlot_t *)0) { *written_slot = target; }
    return UPDATE_HANDOFF_STORAGE_OK;
}
