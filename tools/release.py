...
from tools.secure_container import (  # noqa: E402
    IMAGE_DELTA,
    IMAGE_FULL,
    pack_header,
    sign_ecdsa_p256,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create an immutable signed firmware release"
    )
    ...
    try:
        ...
        print(
            f"RELEASE=PASS release={verified.release_id} "
            f"target=v{verified.target_version} artifacts={len(verified.artifacts)}"
        )
        print(f"RELEASE_KEY_ID=0x{verified.key_id:08X}")
        print(
            "RELEASE_ARTIFACTS="
