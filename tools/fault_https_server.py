#!/usr/bin/env python3
"""TLS artifact server with deterministic HIL transport faults."""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt: str, *args: object) -> None:
        print(
            f'HIL_HTTPS peer={self.client_address[0]} '
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if self.server.mode == "truncate":
            ...
            self.wfile.flush()
            print(
                f"HIL_HTTPS_FAULT=TRUNCATE sent={cut} total={len(data)}",
                flush=True,
            )
            try:
        ...
        self.wfile.flush()
        print(
            f"HIL_HTTPS_ARTIFACT=PASS size={len(data)} mode={self.server.mode}",
            flush=True,
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Deterministic TLS artifact fault server for HIL"
    )
    ...
    parser.add_argument(
        "--route",
        default="/fault-injection HIL/artifact.sdot",
    )
    parser.add_argument(
        "--mode",
    ...
    print(
        f"HIL_HTTPS_READY bind={args.bind} port={args.port} "
    ...
    try:
        # One request is enough for each deterministic fault-injection HIL scenario.
    ...
    if server.served_count != 1:
        print("HIL_HTTPS_RESULT=FAIL artifact_not_served", flush=True)
        return 1

    print(
        f"HIL_HTTPS_RESULT=PASS mode={args.mode}",
        flush=True,
    )
    return 0
