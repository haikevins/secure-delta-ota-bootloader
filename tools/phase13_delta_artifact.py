#!/usr/bin/env python3
"""Wrap a Phase-12 .jdiff patch in the Phase-13 functional delta envelope."""
...
def parse_header(raw: bytes) -> dict[str, int]:
    if len(raw) < HEADER_SIZE:
        raise ValueError("artifact shorter than Phase-13 delta header")

    values = HEADER_WITHOUT_CRC.unpack_from(raw)
    stored_crc = HEADER_CRC.unpack_from(raw, HEADER_SIZE - 4)[0]

    if values[0] != MAGIC:
        raise ValueError("bad Phase-13 delta magic")
    if values[1] != FORMAT_VERSION or values[2] != HEADER_SIZE:
        raise ValueError("bad Phase-13 delta header version/size")
    if stored_crc != crc32(raw[:HEADER_SIZE - 4]):
        raise ValueError("Phase-13 delta header CRC mismatch")
...
def main() -> int:
    ...
    parser.add_argument(
        "--patch",
        type=Path,
        default=root / "dist/phase13/application-v1-to-v2.jdiff",
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=root / "dist/phase13/application-v1-to-v2.d13",
    )
    ...
    if parsed["patch_size"] != len(patch):
        raise SystemExit("internal Phase-13 patch-size mismatch")
    if parsed["base_image_crc32"] != crc32(base):
        raise SystemExit("internal Phase-13 base CRC mismatch")
    if parsed["target_image_crc32"] != crc32(target):
        raise SystemExit("internal Phase-13 target CRC mismatch")
    if parsed["patch_crc32"] != crc32(patch):
        raise SystemExit("internal Phase-13 patch CRC mismatch")
    ...
    meta = {
        ...
        "security_note": (
            "CRC32 provides Phase-13 integrity/base binding only; "
            "signed authenticity remains Phase 14."
        ),
    }
    ...
    print(
        f"PHASE13_ARTIFACT=PASS size={len(artifact)} "
        f"header={HEADER_SIZE} patch={len(patch)}"
    )
    print(f"PHASE13_ARTIFACT_CRC32=0x{crc32(artifact):08X}")
    print(f"PHASE13_BASE_CRC32=0x{crc32(base):08X}")
    print(f"PHASE13_TARGET_CRC32=0x{crc32(target):08X}")
    print(f"PHASE13_PATCH_CRC32=0x{crc32(patch):08X}")
    print(f"PHASE13_ARTIFACT_PATH={output}")
    return 0
