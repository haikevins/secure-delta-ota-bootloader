...
def sign_ecdsa_p256(private_key: Path, signed_bytes: bytes) -> bytes:
    with tempfile.TemporaryDirectory(prefix="phase14-sign-") as td:
        td_path = Path(td)
...
def verify_openssl(
    ...
    with tempfile.TemporaryDirectory(prefix="phase14-verify-") as td:
        td_path = Path(td)
        message = td_path / "signed.bin"
...
def build_container(args: argparse.Namespace) -> int:
    ...
    print(
        f"PHASE14_CONTAINER=PASS type={args.type} "
        f"base=v{args.base_version} target=v{args.target_version} "
        f"payload={len(payload)} container={len(container)}"
    )
    print(f"PHASE14_KEY_ID=0x{args.key_id:08X}")
    print(f"PHASE14_CONTAINER_SHA256={sha256(container).hex()}")
    print(f"PHASE14_SIGNED_SHA256={sha256(signed_bytes).hex()}")
    print("PHASE14_OPENSSL_VERIFY=PASS")
    return 0
