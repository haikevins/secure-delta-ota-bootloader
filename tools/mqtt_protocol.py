...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Minimal TLS MQTT 3.1.1 broker for gateway MQTT HIL test"
    )
    ...
    print(
        f"gateway MQTT_MQTT_BROKER_READY bind={args.bind} "
    ...
    try:
        ...
        with raw_conn:
            with context.wrap_socket(
                ...
                print(
                    f"MQTT_BROKER_TLS=PASS peer={peer[0]}:{peer[1]}",
                    flush=True,
                )
                ...
                while True:
                    ...
                    if packet_type == 1:  # CONNECT
                        ...
                        print("MQTT_BROKER_CONNECT=PASS", flush=True)
                        ...
                        if topic == status_topic:
                            ...
                            print(
                                f"MQTT_BROKER_PROGRESS {text}",
                                flush=True,
                            )
                        else:
                            print(
                                f"MQTT_BROKER_PUBLISH topic={topic} "
                                f"payload={text}",
                                flush=True,
                            )
                        ...
                        print("MQTT_BROKER_DISCONNECT=PASS", flush=True)
                        break

                    else:
                        print(
                            f"MQTT_BROKER_PACKET type={packet_type}",
                            flush=True,
                        )

    except (EOFError, ConnectionResetError, socket.timeout) as exc:
        print(f"MQTT_BROKER_CONNECTION_END {exc}", flush=True)
    finally:
        listener.close()

    if not command_sent:
        print("MQTT_BROKER_RESULT=FAIL command_not_sent", flush=True)
        return 1
    if not command_acked:
        print("MQTT_BROKER_RESULT=FAIL command_not_acked", flush=True)
        return 1
    if not confirmed_seen:
        print("MQTT_BROKER_RESULT=FAIL confirmed_status_missing", flush=True)
        return 1

    print("MQTT_BROKER_RESULT=PASS", flush=True)
    return 0
