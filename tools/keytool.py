...
def public_key_raw(private_or_public_key: Path) -> bytes:
    with tempfile.TemporaryDirectory(prefix="signed secure container-key-") as td:
    ...
    if (
        ...
        raise SystemExit(
            "signed secure container requires an ECDSA P-256/prime256v1 key"
        )
    ...
    output.write_text(
        "\n".join(
            [
                ...
                "#include <stdint.h>",
                "",
                "#define TRUSTED_KEY_PROVISIONED 1U",
                f"#define TRUSTED_KEY_ID          0x{key_id:08X}UL",
                "",
...
def main() -> int:
    ...
    print(
        f"KEY_PROVISION=PASS key_id=0x{args.key_id:08X} "
