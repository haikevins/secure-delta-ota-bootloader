#!/usr/bin/env python3
"""Generate and verify a deterministic JojoDiff-compatible firmware delta."""
...
def run_external_applier(executable: Path,
                         ...
                         patch: Path,
                         target: bytes) -> None:
    with tempfile.TemporaryDirectory(prefix="delta-external-") as td:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Deterministic JojoDiff-compatible delta generator"
    )
    parser.add_argument(
        "--base",
        type=Path,
    ...
    if reconstructed != target:
        raise SystemExit("delta generation internal reconstruction mismatch")
    ...
    print(
        f"DELTA=PASS base=v{args.base_version} "
        f"target=v{args.target_version}"
    )
    print(
        f"DELTA_SIZES base={len(base)} target={len(target)} "
        f"patch={len(patch)}"
    )
    print(
        f"DELTA_SAVINGS={savings_percent:.2f}% "
        f"threshold={args.min_savings_percent:.2f}% "
        f"eligible={'yes' if eligible else 'no'}"
    )
    print(f"DELTA_BASE_SHA256={sha256_bytes(base)}")
    print(f"DELTA_TARGET_SHA256={sha256_bytes(target)}")
    print(f"DELTA_PATCH_SHA256={sha256_bytes(patch)}")
    print(
        f"DELTA_PATCH_CRC32=0x"
        f"{zlib.crc32(patch) & 0xFFFFFFFF:08X}"
    )
    print(f"DELTA_PATCH={patch_path}")
    print(f"DELTA_MANIFEST={manifest_path}")
    print("DELTA_ROUNDTRIP=PASS byte-for-byte")
    if external_checked:
        print("DELTA_EXTERNAL_JANPATCH=PASS")
    else:
        print(
            "DELTA_EXTERNAL_JANPATCH=SKIPPED "
