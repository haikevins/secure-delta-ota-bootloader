#!/usr/bin/env python3
"""Multi-connection TLS MQTT fault broker for deterministic HIL.
...
import mqtt_protocol as mqtt_proto
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Reconnect-capable TLS MQTT fault broker for deterministic HIL"
    )
    ...
    print(
        f"HIL_MQTT_BROKER_READY bind={args.bind} port={args.port} "
    ...
    try:
        while (
            ...
            try:
                ...
                print("HIL_BROKER_ACCEPT_TIMEOUT", flush=True)
                break
            ...
            with raw_conn:
                try:
                    with context.wrap_socket(
                        ...
                        print(
                            f"HIL_BROKER_TLS=PASS connection={connection_count} "
                        ...
                        while True:
                            first, body = mqtt_proto.recv_packet(conn)
                            packet_type = first >> 4

                            if packet_type == 1:  # CONNECT
                                mqtt_proto.send_packet(conn, 0x20, b"\x00\x00")
                                print(
                                    f"HIL_BROKER_CONNECT=PASS "
                                    f"connection={connection_count}",
                                    flush=True,
                                )

                            elif packet_type == 8:  # SUBSCRIBE
                                sub_id, topics = mqtt_proto.parse_subscribe(body)
                                granted = bytearray()
                                ...
                                topic, payload, pub_id = mqtt_proto.parse_publish(
                                    first, body
                                )
                                qos = (first >> 1) & 0x03
                                text = payload.decode(
                                ...
                                if topic == status_topic:
                                    ...
                                    print(
                                        f"HIL_BROKER_PUBLISH topic={topic} "
                                        f"payload={text}",
                                        flush=True,
                                    )
                                ...
                                mqtt_proto.send_packet(conn, 0xD0)

                            elif packet_type == 14:  # DISCONNECT
                                print(
                                    f"HIL_BROKER_DISCONNECT=PASS "
                                    f"connection={connection_count}",
                                    flush=True,
                                )
                                break
    ...
    if not ever_command_sent:
        print("HIL_BROKER_RESULT=FAIL command_not_sent", flush=True)
        return 1
    if not ever_command_acked:
        print("HIL_BROKER_RESULT=FAIL command_not_acked", flush=True)
        return 1
    if args.disconnect_on_state != "none" and not injected_disconnect:
        print("HIL_BROKER_RESULT=FAIL fault_not_injected", flush=True)
        return 1
    if not final_seen:
        print(
            f"HIL_BROKER_RESULT=FAIL "
    ...
    print(
        f"HIL_BROKER_RESULT=PASS final={args.expected_final} "
