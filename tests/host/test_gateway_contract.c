...
int main(void)
{
    ...
    if (status != 0)
    {
        fprintf(stderr, "release pipeline full contract failed: %d\n", status);
        return status;
    }
    ...
    if (status != 0)
    {
        fprintf(stderr, "release pipeline delta contract failed: %d\n", status);
        return status;
    }

    puts("release pipeline ESP32 secure-container UART contract: PASS");
    return 0;
}
