#!/usr/bin/env python3
"""Reproducible Phase-17 build/artifact benchmark for the portfolio."""
...
def build_component(
    ...
    *,
    ...
    build_dir = f"build-phase17-bench-{label}"
    out_dir = f"out-phase17-bench-{label}"
    try:
        _, clean_time = run(
            [
                "make",
        ...
        copy_dir = ROOT / ".phase17-benchmark-tmp"
...
def create_signed_release(
    *,
    ...
    key = temp / "phase17-benchmark-key.pem"
    run(
        [
            "openssl",
            "ecparam",
    ...
    release = out_root / "phase17-benchmark"
...
def write_outputs(results: dict[str, object], output_dir: Path) -> None:
    ...
    (output_dir / "phase17_benchmark.json").write_text(
    ...
    row("phase16_hil_passed", hil["passed"], "scenarios")
    row("phase16_hil_total", hil["total"], "scenarios")
    row("owned_source_files", codebase["files"], "files")
    row("owned_nonblank_lines", codebase["nonblank_lines"], "lines")

    with (output_dir / "phase17_benchmark.csv").open(
    ...
    md = [
        ...
        "",
        f"- Phase 16 deterministic HIL: **{hil['passed']}/{hil['total']} PASS**.",
    ...
    (output_dir / "phase17_benchmark.md").write_text(
        "\n".join(md),
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=9,
        help="Phase-16 deterministic scenarios observed PASS",
    )
    ...
    try:
        ...
        temp_copy_dir = ROOT / ".phase17-benchmark-tmp"
        ...
        with tempfile.TemporaryDirectory(prefix="phase17-release-") as td:
        ...
        print(
            "PHASE17_HIL_EVIDENCE=PASS "
            f"{args.hardware_hil_passed}/{args.hardware_hil_total}"
        )
        print(f"PHASE17_OUTPUT={output_dir}")
        return 0
    except (BenchmarkError, OSError, subprocess.TimeoutExpired) as exc:
        print(f"Phase 17 benchmark: FAIL: {exc}")
        return 1
    finally:
        shutil.rmtree(ROOT / ".phase17-benchmark-tmp", ignore_errors=True)
