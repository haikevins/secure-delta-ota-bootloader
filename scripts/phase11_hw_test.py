...
STM32_BASELINE = ROOT / "dist/secure-delta-ota-phase11.bin"
CANDIDATE = (
    ROOT
    / "node-stm32f103/application/out-phase11-candidate/application.bin"
)
...
MQTT_BROKER = ROOT / "tools/phase11_mqtt_broker.py"
...
PASS_MARKER = "P11_GATEWAY_HW_TEST=PASS"
FAIL_MARKER = "P11_GATEWAY_HW_TEST=FAIL"
...
def fail(message: str) -> None:
    print(f"Phase 11 hardware test: FAIL: {message}")
    raise SystemExit(1)
...
def detect_host_ip() -> str:
    explicit = (
        os.environ.get("PHASE11_HOST_IP", "").strip()
    ...
    try:
        ...
        fail(
            ...
            f"PHASE11_HOST_IP manually ({exc})"
        )
    finally:
        sock.close()
...
def generate_test_pki(directory: Path,
    ...
    if not openssl:
        fail("openssl is required for Phase-11 TLS hardware test")
    ...
    run_checked([
        ...
        "-sha256",
        "-days", "2",
        "-subj", "/CN=Secure Delta OTA Phase11 Test CA",
        "-keyout", str(ca_key),
        "-out", str(ca_pem),
    ], stage="generate Phase-11 test CA")
    ...
    run_checked([
        ...
        "-out", str(server_csr),
    ], stage="generate Phase-11 TLS server CSR")
    ...
    run_checked([
        ...
        "-out", str(server_pem),
    ], stage="sign Phase-11 TLS server certificate")
...
def write_hardware_runtime_config(ssid: str,
    ...
    RUNTIME_CONFIG.write_text(
        "\n".join([
            ...
            f"#define SDOTA_PHASE11_HW_WIFI_SSID         {c_string(ssid)}",
            f"#define SDOTA_PHASE11_HW_WIFI_PASSWORD     {c_string(password)}",
            f"#define SDOTA_PHASE11_HW_MQTT_URI          {c_string(mqtt_uri)}",
            "#define SDOTA_PHASE11_HW_USE_TEST_CA       1",
            f"#define SDOTA_PHASE11_HW_TEST_EPOCH        {epoch}LL",
    ...
    with serial.Serial(
        ...
        while time.monotonic() < deadline:
            ...
            if PASS_MARKER in line:
                ...
                if missing:
                    fail(
                        "Phase-11 PASS arrived without required markers: "
                        f"{sorted(missing)}"
                    )
                return observed

    fail("timed out waiting for ESP32 Phase-11 PASS marker")
    raise AssertionError
...
def verify_stm32_final(openocd: str,
    ...
    with tempfile.TemporaryDirectory(prefix="phase11-verify-") as td:
        td_path = Path(td)
        ...
        run_openocd(
            ...
            "resume; shutdown",
            "STM32 final Phase-11 verification",
        )
        ...
        print(
            f"P11_STM32_METADATA generation={fields[1]} "
...
def main() -> int:
    ...
    if not idf or not os.environ.get("IDF_PATH"):
        fail(
            ...
            "Source your ESP-IDF export.sh before Phase 11."
        )
    ...
    for path in (
        ...
        if not path.is_file():
            fail(f"missing Phase-11 artifact: {path}")
    ...
    https_url = (
        f"https://{host_ip}:{https_port}/phase11_candidate.bin"
    )
    mqtt_uri = f"mqtts://{host_ip}:{mqtt_port}"

    print(
        "Phase 11 test: MQTT command -> HTTPS -> ESP32 cache "
        "-> UART -> STM32"
    )
    print(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="phase11-hil-") as td:
            test_dir = Path(td)
            ...
            run_openocd(
                ...
                "STM32 Phase-11 baseline program/verify",
            )
            ...
            https_process = start_process(
                [
                    ...
                    "--key", str(server_key),
                    "--file", str(CANDIDATE),
                    "--path", "/phase11_candidate.bin",
                ],
                "Phase-11 HTTPS server",
            )
            ...
            mqtt_process = start_process(
                ...
                "Phase-11 MQTT broker",
            )

            run_checked(
                [idf, "-p", esp32_port, "flash"],
                cwd=GATEWAY,
                stage="ESP32 Phase-11 flash",
                timeout=240,
            )
            ...
            if (
                'GET /phase11_candidate.bin HTTP/1.1" 200'
                not in https_output
            ):
                fail(
                    "local HTTPS server did not observe the "
                    "Phase-11 firmware GET"
                )
        ...
        if mqtt_process is not None:
            stop_process(mqtt_process, "Phase-11 MQTT broker")
        if https_process is not None:
            stop_process(https_process, "Phase-11 HTTPS server")
