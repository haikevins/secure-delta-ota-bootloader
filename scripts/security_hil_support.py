#!/usr/bin/env python3
"""Secure-container HIL support: signed delta, rejection paths and signed full image."""
...
UPDATE_ID_DELTA = 0xD0A10001
UPDATE_ID_TAMPER = 0xD0A10002
UPDATE_ID_FULL = 0xD0A10003
UPDATE_ID_UNSIGNED = 0xD0A10004
UPDATE_ID_DOWNGRADE = 0xD0A10005
...
from uart_ota_sender import (  # noqa: E402
    SECURE_CONTAINER_HEADER_SIZE,
    SerialLink,
    parse_secure_container,
    require_ack,
    secure_ota,
    transfer,
...
def fail(message: str) -> None:
    print(f"signed secure container hardware test: FAIL: {message}")
    raise SystemExit(1)
...
def wait_for_baseline_uart(
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            print(f"SECURITY_UART_SYNC attempt={attempt} {last}")
            if (
            ...
            print(
                f"SECURITY_UART_SYNC_RETRY attempt={attempt} detail={last}"
            )
...
def verify_board(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="signed secure container-stlink-") as td:
        temp = Path(td)
...
def reject_unsigned_full(
    ...
    try:
        ...
        print("UNSIGNED_REJECT=PASS status=SIGNATURE_ERROR")
    finally:
        link.close()
...
def send_tampered_container(
    ...
    info = parse_secure_container(tampered)
    link = open_link(port)
    try:
        transfer(
            link,
...
def reject_downgrade(port: str, signed_v2: bytes) -> None:
    info = parse_secure_container(signed_v2)
    link = open_link(port)
    try:
        ...
        print("DOWNGRADE_REJECT=PASS status=VERSION_REJECTED")
    finally:
        link.close()
...
def main() -> int:
    ...
    if not port:
        print("Usage: make signed secure container-hw-test PORT=/dev/ttyUSB0")
        return 2
    ...
    try:
        ...
        fail(
            "SDOTA_HIL_KEY_ID must be a valid integer, for example "
    ...
    print(
        "signed secure container HIL: signed delta v1->v2, unsigned rejection, "
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="signed secure container-hw-") as td:
            temp = Path(td)
            ...
            if external_private:
                ...
                if not private_key.is_file():
                    fail(
                        "SDOTA_SIGNING_KEY does not exist: "
                        f"{private_key}"
                    )
            ...
            print(
                f"HIL_KEY=PASS mode={key_mode} "
                f"key_id=0x{key_id:08X}"
            )

            v1_path = build_application(
                toolchain, 1, "build-security-hw-v1", "out-security-hw-v1"
            )
            v2_path = build_application(
                toolchain, 2, "build-security-hw-v2", "out-security-hw-v2"
            )
            v3_path = build_application(
                toolchain, 3, "build-security-hw-v3", "out-security-hw-v3"
            )
            ...
            if bootloader.stat().st_size > 24 * 1024:
                fail(
                    "provisioned signed secure container bootloader exceeds 24 KiB: "
            ...
            baseline = temp / "secure-delta-ota-security-baseline.bin"
            ...
            run(
                [
                    ...
                    str(baseline),
                    "--label",
                    "signed secure container secure HIL baseline",
                ]
            )
            ...
            print(
                f"HIL_ARTIFACT delta={len(delta_bytes)} "
            ...
            run_openocd(
                ...
                "signed secure container baseline program/verify",
            )
            run_openocd(
                openocd,
                scripts,
                ...
                "signed secure container metadata erase/baseline boot",
            )
            ...
            try:
                ...
                print(
                    f"HIL_BASELINE=PASS app=v{initial.application_version} "
                    f"caps=0x{initial.capability_flags:08X}"
                )

                secure_ota(link, delta_bytes, UPDATE_ID_DELTA)
                print("SECURE_DELTA=PASS target=v2")
            except (OSError, TimeoutError, ValueError) as exc:
            ...
            try:
                ...
                print("SECURE_FULL=PASS target=v3")
            except (OSError, TimeoutError, ValueError) as exc:
            ...
            print(
                "signed secure container secure container hardware test: PASS "
        ...
        for name in (
            "build-security-hw-v1",
            "out-security-hw-v1",
            "build-security-hw-v2",
            "out-security-hw-v2",
            "build-security-hw-v3",
            "out-security-hw-v3",
        ):
            path = APP / name
            if path.exists():
