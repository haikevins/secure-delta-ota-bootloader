#!/usr/bin/env python3
"""Deterministic HIL faults across signed release -> ESP32 -> STM32."""
...
import release_hil_support as release_support  # noqa: E402
...
def fail(message: str) -> None:
    print(f"HIL hardware test: FAIL: {message}")
    raise SystemExit(1)
...
def build_application(
    ...
    build_dir = f"build-hil-{label}"
    out_dir = f"out-hil-{label}"
...
def build_bootloader(
    ...
    if image.stat().st_size > 24 * 1024:
        fail(
            f"{label} bootloader exceeds 24 KiB: "
            f"{image.stat().st_size}"
        )
    print(
        f"HIL_BOOTLOADER=PASS label={label} "
    ...
    *,
    ...
    with serial.Serial(
        ...
        while time.monotonic() < deadline:
            ...
            if "GATEWAY_HIL=PASS" in line:
                if not expected_pass:
                    fail("gateway unexpectedly PASSed a negative fault scenario")
                missing = required - observed
                if missing:
                    fail(
                        "gateway PASS missing markers: "
                        + ", ".join(sorted(missing))
                    )
                return "\n".join(lines)

            if "GATEWAY_HIL=FAIL" in line:
                if expected_pass:
...
def dump_stm32(
    ...
    security_support.run_openocd(
        ...
        scripts,
        "init; halt; "
        f"dump_image {security_support.tcl_path(metadata_path)} "
        f"0x{METADATA_ADDRESS:08X} 0x{METADATA_SIZE:X}; "
        f"dump_image {security_support.tcl_path(app_path)} "
        f"0x{APPLICATION_START:08X} 0x{APPLICATION_REGION_SIZE:X}; "
        "resume; shutdown",
        f"{label} STM32 snapshot",
    )

    fields = gateway_support.select_metadata(metadata_path.read_bytes())
    ...
    print(
        f"STM32_METADATA label={label} generation={fields[1]} "
...
def verify_state(
    ...
    *,
    ...
    print(f"STM32_VERIFY=PASS label={label} app=v{expected_version}")
...
def validate_broker_output(
    ...
    *,
    ...
    required = [
        "HIL_BROKER_TLS=PASS",
        "HIL_BROKER_CONNECT=PASS",
        "HIL_BROKER_SUBSCRIBE=PASS",
        "HIL_BROKER_COMMAND_SENT=PASS",
        "HIL_BROKER_COMMAND_ACK=PASS",
        f"HIL_BROKER_FINAL=PASS state={expected_final}",
        f"HIL_BROKER_RESULT=PASS final={expected_final}",
    ]
    if expect_disconnect_fault:
        required.append("HIL_MQTT_FAULT=DISCONNECT")

    missing = [token for token in required if token not in output]
    if missing:
        fail(
            "MQTT transcript missing: "
            + ", ".join(missing)
        )
...
def program_baseline(
    ...
    run([
        ...
        "--label", f"{label} external sanitizer",
    ])

    security_support.run_openocd(
        openocd,
        scripts,
        f"program {security_support.tcl_path(sanitizer_image)} verify exit 0x08000000",
        f"{label} external-metadata sanitizer program/verify",
    )
    security_support.run_openocd(
        openocd,
        scripts,
        ...
        f"{label} external-metadata/incoming sanitizer run",
    )
    print(
        f"EXTFLASH_SANITIZE=PASS scenario={label} "
    ...
    security_support.run_openocd(
        openocd,
        scripts,
        f"program {security_support.tcl_path(baseline)} verify exit 0x08000000",
        f"{label} baseline program/verify",
    )
    security_support.run_openocd(
        openocd,
        scripts,
        ...
        f"{label} metadata erase/baseline boot",
    )
...
def start_production_https(
    ...
    return gateway_support.start_process(
        [
            sys.executable,
            "server/app/main.py",
            "serve",
...
def start_fault_https(
    ...
    return gateway_support.start_process(
        [
            ...
            "--route", "/fault-injection HIL/artifact.sdot",
            "--mode", mode,
            "--truncate-after", "512",
        ],
        f"{mode} HTTPS fault server",
    )
    ...
    *,
    ...
    if label in ("rollback-control", "rollback-reset"):
        ...
        write_command(
            ...
            url=f"https://{host_ip}:{https_port}/fault-injection HIL/artifact.sdot",
        )
    ...
    try:
        ...
        required = {
            "WIFI_PS=PASS mode=none",
            "MQTT=PASS",
            "OTA_COMMAND=PASS",
        }
        if label != "https-truncate":
            required.add("OTA_HTTPS=PASS")
        if expected_pass:
            required.update({
                "OTA_FINAL=PASS",
    ...
    if label == "https-truncate":
        if "HIL_HTTPS_FAULT=TRUNCATE" not in https_output:
            fail("HTTPS truncation scenario did not inject the cut")
    elif label == "tampered-signature":
        if "HIL_HTTPS_ARTIFACT=PASS" not in https_output:
    ...
    print(
        f"SCENARIO=PASS id={label} generation={fields[1]} "
...
def main() -> int:
    ...
    if not esp32_port or not wifi_ssid:
        print(
            "Usage: make fault-injection HIL-hw-test "
    ...
    openocd, openocd_scripts = security_support.resolve_stm32_openocd()
    ...
    matrix = json.loads(
        (ROOT / "tests/fault/hil_fault_matrix.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    print(
        "HIL: deterministic secure OTA fault injection "
        f"({len(scenarios)} scenarios)"
    )
    print(
        "Wiring remains release pipeline: ESP32 GPIO17 TX -> STM32 PA10 RX, "
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="fault-injection HIL-hil-") as td:
            temp = Path(td)
            private_key = temp / "fault-injection HIL-release-private.pem"
            ...
            bad_manifest, bad_artifact = create_release(
                ...
                release_root,
                "fw-v2-bad",
                "fault-injection HIL-unconfirmed",
            )

            # Create a transport-valid but signature-invalid SDOT outside the
            # immutable verified release directory. The fault server
            ...
            release_support.verify_runtime_mqtt_uri(mqtt_uri)
            ...
            release_support.verify_built_gateway_mqtt_uri(mqtt_uri)

            for index, scenario in enumerate(scenarios, start=1):
                update_id = 0xD0A20000 + index
        ...
        for pattern in ("build-hil-*", "out-hil-*"):
