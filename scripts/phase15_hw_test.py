#!/usr/bin/env python3
"""Phase 15 HIL: signed release server -> MQTTS -> ESP32 -> HTTPS -> UART -> STM32."""
...
def fail(message: str) -> None:
    print(f"Phase 15 hardware test: FAIL: {message}")
    raise SystemExit(1)
...
def detect_host_ip() -> str:
    ...
    if explicit:
        try:
            ...
            fail(f"PHASE15_HOST_IP is not valid IPv4: {explicit}")
    ...
    try:
        ...
        fail(
            "could not auto-detect PC LAN IPv4; set PHASE15_HOST_IP "
            f"manually ({exc})"
        )
    finally:
        sock.close()
...
def verify_runtime_mqtt_uri(expected_uri: str) -> None:
    ...
    required = [
        "#define SDOTA_PHASE11_HW_OVERRIDE          1",
        f"#define SDOTA_PHASE11_HW_MQTT_URI          {escaped}",
        "#define SDOTA_PHASE11_HW_USE_TEST_CA       1",
        "#define SDOTA_PHASE11_HW_SINGLE_SHOT       1",
    ]

    for token in required:
        if token not in text:
            fail(
                "generated Phase-15 ESP32 runtime config mismatch; "
                f"missing {token!r}"
            )

    print(f"P15_RUNTIME_CONFIG=PASS mqtt_uri={expected_uri}")
...
def verify_built_gateway_mqtt_uri(expected_uri: str) -> None:
    ...
    print(
        f"P15_COMPILED_MQTT_URI=PASS mqtt_uri={expected_uri} "
...
def main() -> int:
    ...
    if not esp32_port or not wifi_ssid:
        print(
            "Usage: make phase15-hw-test "
            "ESP32_PORT=/dev/ttyUSB0 "
            'WIFI_SSID="your-ssid" WIFI_PASSWORD="your-password" '
            "PHASE15_HOST_IP=<PC-LAN-IP>"
        )
        return 2
    ...
    stm32_openocd, stm32_scripts = p14.resolve_stm32_openocd()
    host_ip = detect_host_ip()
    https_port = parse_port("HTTPS_PORT", 8443)
    mqtt_port = parse_port("MQTT_PORT", 8883)

    raw_key_id = os.environ.get("PHASE15_KEY_ID", "").strip()
    if not raw_key_id:
        raw_key_id = f"0x{KEY_ID_DEFAULT:08X}"
    try:
        key_id = int(raw_key_id, 0)
    except ValueError:
        fail(f"PHASE15_KEY_ID invalid: {raw_key_id!r}")
    if not 1 <= key_id <= 0xFFFFFFFF:
        fail("PHASE15_KEY_ID must fit non-zero uint32")

    print(
        "Phase 15 HIL: signed release -> MQTTS command -> HTTPS cache -> "
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="phase15-hil-") as td:
            temp = Path(td)
            ...
            base = p14.build_application(
                toolchain,
                BASE_VERSION,
                "build-phase15-hw-v1",
                "out-phase15-hw-v1",
            )
            target = p14.build_application(
                toolchain,
                TARGET_VERSION,
                "build-phase15-hw-v2",
                "out-phase15-hw-v2",
            ...
            baseline = temp / "secure-delta-ota-phase15-baseline.bin"
            run([
                sys.executable,
                ...
                "--output", str(baseline),
                "--label", "Phase 15 secure server HIL baseline",
            ])
            ...
            p14.run_openocd(
                stm32_openocd,
                stm32_scripts,
                f"program {p14.tcl_path(baseline)} "
                "verify exit 0x08000000",
                "STM32 Phase-15 baseline program/verify",
            )
            p14.run_openocd(
                stm32_openocd,
                stm32_scripts,
                ...
                "reset run; shutdown",
                "STM32 Phase-15 metadata erase/baseline boot",
            )

            https_process = p11.start_process(
                [
                    sys.executable,
                    "server/app/main.py",
                    "serve",
            ...
            # Gateway retains P11 log markers because the UART/MQTT transport
            # contract is unchanged; Phase 15 changes the release artifact.
            p11.monitor_esp32(esp32_port, timeout_s=180.0)

            p11.verify_stm32_final(
                stm32_openocd,
                stm32_scripts,
                target.read_bytes(),
            )
            ...
            print("P15_HTTPS_RELEASE_GET=PASS")

            p11.validate_broker_output(mqtt_output)
            print(
                f"P15_RELEASE_PIPELINE=PASS artifact={artifact.kind} "
        ...
        if mqtt_process is not None:
            p11.stop_process(mqtt_process, "Phase-15 MQTTS broker")
        if https_process is not None:
            p11.stop_process(https_process, "Phase-15 HTTPS release server")
    ...
    print(
        ...
        "the Phase-15 signed release; release private key not persisted."
    )
    return 0
